<template>
	<view class="classify">
		...
		<!-- 内容 -->
		<view class="list_box">
			<view class="boxItem" v-for="(item,index) in historyData" :key="index" @tap="gotoDetail(item.itemid)">
				<view class="imgWrap">
...
<script>
	...
	export default {
		...
		// 完成挂载
		mounted() {
			this.historyData = this.$queue.get(this.historyKey);
			this.noData = this.historyData.length>0 ? false : true
		},
